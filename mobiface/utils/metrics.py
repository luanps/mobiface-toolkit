...
def norm_center_error(rects1, rects2, width, height):
    r"""Normalised center error.
    ...
    diff = centers1 - centers2
    normalised_diff = diff / size
    
    errors = np.sqrt(np.sum(np.power(normalised_diff, 2), axis=-1))

    # error=0 where the prediction and the annotations are all [0,0,0,0]
    sum1 = np.sum(rects1, axis=1)
    sum2 = np.sum(rects2, axis=1)
    idx1 = np.where(sum1==0)[0]
    idx2 = np.where(sum2==0)[0]
...
def rect_iou(rects1, rects2, bound=None):
    ...
    # iou=1 where the prediction and the annotations are all [0,0,0,0]
    sum1 = np.sum(rects1, axis=1)
    sum2 = np.sum(rects2, axis=1)
    idx1 = np.where(sum1==0)[0]
    idx2 = np.where(sum2==0)[0]
...
if __name__ == '__main__':
    r1 = np.array([21,3,2,11])
    r1 = np.array([[0]*4, [0]*4, [100,100,50,50]])
    r2 = np.array([[0]*4, [0]*4, [100,120,70,20]])
    print(rect_iou(r1,r2))
